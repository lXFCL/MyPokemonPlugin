...
public class CreateDatabase {
    ...
    public static String tableName = INSTANCE.getConfig().getString("MySQL.tableName");
    public static String serverLimitTableName = INSTANCE.getConfig().getString("MySQL.serverLimitTableName");
    String[] tableNameDataColumn = new String[]{"uuid","shopPosition","shopLimit"};

    String[] serverLimitDataColumn = new String[]{"shopPosition","shopLimit"};
    ...
    public static void cd(){
        if(!tableIsExist(tableName)) {
            sqlManager.createTable(tableName)
                    ...
                    .addColumn("shopPosition","VARCHAR(80) NOT NULL","商品位置")
                    .addColumn("shopLimit", "int NOT NULL", "个人玩家限制购买次数")
                    .build().executeAsync();
            INSTANCE.getLogger().info("§b-> " + tableName + "建表成功!");
        }
        if(!tableIsExist(serverLimitTableName)) {
            sqlManager.createTable(serverLimitTableName)
                    .addColumn("id", "INT PRIMARY KEY AUTO_INCREMENT","目标id")
                    .addColumn("shopPosition","VARCHAR(80) NOT NULL","商品位置")
                    .addColumn("shopLimit", "int NOT NULL", "全服玩家限制购买次数")
                    .build().executeAsync();
            INSTANCE.getLogger().info("§b-> " + serverLimitTableName + "建表成功!");
        }
    }
